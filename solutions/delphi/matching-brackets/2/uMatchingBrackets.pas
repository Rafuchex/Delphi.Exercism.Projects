...
var
  bracketList: TStack<char>;
  i: integer;
begin
  bracketList := TStack<char>.Create;
  Try
    for i := 1 to Length(msg) do
    Begin
      if ((msg[i] = '(') or (msg[i] = '{') or (msg[i] = '[')) then
        bracketList.Push(msg[i])

      else if ((msg[i] = ')') or (msg[i] = '}') or (msg[i] = ']')) then
      Begin
        if not bracketList.IsEmpty then
          if (((bracketList.Peek = '(') and (msg[i] = ')')) or
              ((bracketList.Peek = '[') and (msg[i] = ']')) or
              ((bracketList.Peek = '{') and (msg[i] = '}')))  then
          begin
             bracketList.Pop;
             continue;
          end;
